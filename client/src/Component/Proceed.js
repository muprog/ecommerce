...
export default function Proceed() {
  ...
  const [buyerBuysProduct] = useState()
  const { processedArray } = useContext(userContext)
  ...
  useEffect(() => {
    axios
      .post('/buyer-buys-product', { ...buyerBuysProduct, sum })
      .then((res) => {
        if (res.data.error) {
          setHasError(true)
        } else {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!ethiopianPhoneRegex.test(deliveryData.phone)) {
        return toast.error(
          'Phone number must be valid Ethiopian phone number starting with 09, +251, 251, or 07.'
        )
      }
    ...
    <div>
      {cartData.length === 0 && receipt && noError ? (
        <div className='receipt-whole-div' id='print-receipt'>
          <strong>Buyer</strong> :{' '}
          <span className='receipt-username'>{user.email}</span>
          <table>
            <thead>
              <tr>
                <th>Product name</th>
            ...
            <tbody>
              ...
              <tr>
                <td className='receipt-td total-price'>
                  Total paid:{' '}
                  {new Intl.NumberFormat('en-US', {
                    style: 'decimal',
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  }).format(buyerBuysProduct.totalPrice)}{' '}
                  ETB
                </td>
              </tr>
              <tr>
                <td className='receipt-td'>
                  Payment Date: {buyerBuysProduct.paymentDate}
                </td>
              </tr>{' '}
              {/* Add payment date */}
              <tr>
                <td className='receipt-td'>Payed to: ATAKY.com</td>
              </tr>{' '}
              {/* Add payment date */}
            </tbody>
          </table>
          <div className='receipt-btn'>
            <button onClick={handlePrint} className='pay'>
              Print Receipt
            </button>
            <button onClick={() => navigate('/buyer')} className='pay'>
              Finish
            </button>
          </div>
        </div>
        ...
        <form>
          ...
          <div>
            <button type='button' className='pay' onClick={handlePayment}>
              Pay
            </button>
          </div>
        </form>
