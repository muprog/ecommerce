...
export default function Header() {
  const { user, setSearchController } = useContext(userContext)
  ...
  return (
    <div className='header'>
      <div className='logo-div'>
        <Link to='/'>
          <img src={logo} alt='search icons' className='logo' />{' '}
        </Link>
      </div>
      <div className='header-input-div'>
        ...
        <div className='search-div'>
          <img src={search} alt='search icons' className='search-icon' />
        </div>
      </div>
      {user.email ? (
        <HandleLogout />
