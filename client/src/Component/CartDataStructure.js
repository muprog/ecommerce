...
export default function CartDataStructure(props) {
  ...
  // Removed: some, setSome, filteredData, setFilteredData, someOther, setSomeOther
  ...
  useEffect(() => {
    setProcessedArray(processArray(cartData))
  }, [count, cartData, setProcessedArray]) // ✅ fixed missing dependencies
  ...
  const handlePlus = () => {
    const occurrenceCount = countObjectOccurrencesById(
      cartData,
      props.element._id
    )
    if (occurrenceCount < props.element.quantity) {
      setCartData((prevCartData) => {
        return [...prevCartData, props.element]
      })
    } else {
      toast.error(
        `We have only ${props.element.quantity} number of ${props.element.name}.`
      )
    }
  }
      ...
      <div className='cart-div-except-img'>
        <table>
          <tbody>
            ...
            <tr>
              <td>
                <div className='quantity-ajustment-button'>
            ...
            <tr>
              ...
              <td>
                {processedArray.map((elements) => {
                  ...
                  return null // ✅ prevent map warning
                })}
              </td>
            </tr>
          </tbody>
