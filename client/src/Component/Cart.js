...
export default function Cart() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) =>
        setPreUserBalance(
          res.data.find((Element) => Element.email === user.email)
        )
      )
      .catch((err) => console.log(err))
    ...
    axios
      ...
      .catch((err) => console.log(err))
  }, [navigate, setPreUserBalance, user]) // ✅ fixed missing dependencies
  ...
  useEffect(() => {
    setUniqueArray(getUniqueArray(cartData))
  }, [cartData, setUniqueArray]) // ✅ fixed missing dependency
      ...
      <div className='cart-whole-div'>
        ...
        {cartData.length === 0 ? (
          ...
          <div className='payment'>
            ...
            <div>
              Total Price <span className='span'>ETB {sum}</span>
            </div>
            <div>
