...
const registerUser = async (req, res) => {
  try {
    ...
    if (!password || password.length < 6) {
      return res.json({
        error: 'Password is required or it should be at least 6 character.',
      })
    }
...
const verifyOtp = async (req, res) => {
  const { email, otp, fname, lname, password, phone, usertype, status } =
    req.body
...
const loginUser = async (req, res) => {
  try {
    ...
    if (user.status === 'off')
      return res.json({ error: 'Your status is inactive' })
  ...
  try {
    ...
    if (description) {
      if (description.length > 300) {
        return res.json({
          error: 'The length of description must be less than 300.',
        })
      }
    }
...
const buyerBuysProduct = async (req, res) => {
  try {
    ...
    if (!buyer || !totalPrice || !buys || !address || !phone) {
      return res.json({
        error: 'buyer total price and the products are required',
      })
    } else if (!/^(?:\+251|251|0)?(9|7)\d{8}$/.test(phone)) {
      return res.json({ error: 'Phone number must be valid Ethiopian Number' })
    }
    ...
    if (userBalance) {
      if (userBalance.balance > totalPrice) {
        const buyerBuysProduct = await BuyerBuysProduct.create({
          buyer,
          buys,
          address,
          phone,
...
const updateDeliveryStatus = async (req, res) => {
  try {
    const { status, id } = req.body
    if (status === 'unChecked') {
      const buyerBuysProduct = await BuyerBuysProduct.findByIdAndUpdate(
        { _id: id },
        { status: 'Checked' }
      )
...
const createAdder = async (req, res) => {
  try {
    ...
    if (!phone) {
      ...
      return res.json({
        error: 'Phone number must be a valid Ethiopian number',
      })
    }

    if (!password || password.length < 6) {
      return res.json({
        error: 'Password is required or it should be at least 6 characters.',
      })
    }
    ...
    if (!usertype) {
      return res.json({
        error: 'Your role (seller, buyer, or adder) is required.',
      })
    }
...
const categoryAndType = async (req, res) => {
  try {
    ...
    const categoryAndType = await CategoryAndType.create({
...
const Favorite1 = async (req, res) => {
  const { productId, userEmail, name, brand, size, price, quantity, photo } =
    req.body
